...
void remove_child_process (struct thread *cp)
{
	list_remove(&cp->child_elem);		// remove from child_list
	palloc_free_page(cp);			// Delete Process Descriptor
}
void argument_stack(char **parse, int count, void **esp)
{
	int i,j;
...
{
  char *fn_copy, *file_name_copy, *thread_name, *saveptr;
  tid_t tid;
...
start_process (void *file_name_)
{
  ...
  char *file_name = file_name_, **argv, *token, *saveptr;
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  /* Wait for Child Process's exit as use exit_sema*/
...
bool handle_mm_fault (struct vm_entry *vme)
{
	...
	vme->is_loaded = install_page(vme->vaddr, kaddr, vme->writable);
